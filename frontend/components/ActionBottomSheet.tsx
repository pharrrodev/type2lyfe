...
const ActionBottomSheet: React.FC<ActionBottomSheetProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* Sheet */}
      <div className={`bg-card dark:bg-slate-800 rounded-t-3xl shadow-modal w-full max-w-lg p-6 pt-4 transition-transform duration-300 ease-in-out transform absolute bottom-0 left-1/2 -translate-x-1/2 border-t border-border dark:border-slate-700 ${isOpen ? 'translate-y-0' : 'translate-y-full'}`}>
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-semibold text-text-primary dark:text-slate-100">What would you like to log?</h3>
          <button onClick={onClose} className="p-2 text-text-secondary dark:text-slate-400 hover:bg-background dark:hover:bg-slate-700 rounded-full transition-all duration-300">
        ...
        <div className="grid grid-cols-3 gap-3">
          <ActionButton
            icon={<DropletIcon className="w-8 h-8 text-info" />}
            label="Glucose"
            onClick={() => handleAction(onLogGlucose)}
          />
          <ActionButton
            icon={<WeightScaleIcon className="w-8 h-8 text-warning" />}
            label="Weight"
            onClick={() => handleAction(onLogWeight)}
          />
          <ActionButton
            icon={<BloodPressureIcon className="w-8 h-8 text-danger" />}
...
const ActionButton: React.FC<{ icon: React.ReactNode; label: string; onClick: () => void }> = ({ icon, label, onClick }) => (
  <button
    onClick={onClick}
    className="flex flex-col items-center justify-center p-4 bg-background dark:bg-slate-700 border border-border dark:border-slate-600 rounded-2xl hover:shadow-card hover:bg-card dark:hover:bg-slate-600 hover:border-primary/40 dark:hover:border-primary/40 transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-primary focus:ring-opacity-50"
  >
    {icon}
